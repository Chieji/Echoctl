...
import { buildExtensionSnapshot, ExtensionToolDescriptor } from '../extensions/snapshot.js';
...
export async function agentTools(): Promise<void> {
  ...
  try {
    ...
    for (const descriptor of Object.values(snapshot.tools)) {
      const tool = descriptor as ExtensionToolDescriptor;
      bySource[tool.source].push(tool.name);
    }
