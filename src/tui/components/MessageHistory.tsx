 ...
 * ECHOMEN TUI Components - MessageHistory
 * Displays conversation history with syntax highlighting
 * Optimized for high-frequency updates (Performance: Bolt ⚡)
 */
      ...
      {visibleMessages.length === 0 && !streamingMessage ? (
        ...
        <>
          {visibleMessages.map((msg) => (
            // Use timestamp as key to maintain identity during list shifts (Performance: Bolt ⚡)
...
/**
 * Memoized Message Item to prevent re-renders of old messages during streaming (Performance: Bolt ⚡)
 */
...
// Static regex to avoid re-compilation (Performance: Bolt ⚡)
const CODE_BLOCK_REGEX = /```(\w+)?\n([\s\S]*?)```/g;

/**
 * Memoized Message Content to prevent redundant markdown parsing (Performance: Bolt ⚡)
 */
const MessageContent = React.memo(({ content }: MessageContentProps) => {
  // Memoize parts to avoid re-splitting on every render (Performance: Bolt ⚡)
